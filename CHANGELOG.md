...
- Backend [TRACEFOSS-1458]: BPN Validation for EDC URL mapping in admin view is does not validate BPN
- Backend [TRACEFOSS-589]: Backend API access without login returns HTTP code 500 instead of 401
- Backend [TRACEFOSS-2148]: Parts, Notification endpoints returns unsorted list
---
- Frontend [TRACEFOSS-2149]: Sorting of any empty table on column causes an unhandled error view
---
- Security [TRACEFOSS-829]: Invicti Enterprise detected errors during parsing of Strict-Transport-Security header - The HSTS Warning and Error may allow attackers to bypass HSTS
- Security [TRACEFOSS-830]: Invicti Enterprise identified a stack trace disclosure (Java) in the target web server's HTTP response.
...
---
- Environment [TRACEFOSS-2164]: HTTP Requests for syncing the submodel server do not work
